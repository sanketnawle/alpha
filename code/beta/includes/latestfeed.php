...
function getNewData($con,$user_id){
	...
	if(isset($_POST['latest'])){
		...
		if(isset($_GET['user_id'])) $query = mysqli_query($con,"SELECT * FROM posts WHERE update_timestamp > '".$latest."' AND user_id = ".$_GET['user_id']." ORDER BY update_timestamp DESC");
		else $query = mysqli_query($con,"SELECT * FROM posts WHERE update_timestamp > '".$latest."' ORDER BY update_timestamp DESC");
