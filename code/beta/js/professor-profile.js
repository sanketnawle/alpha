$(document).ready(function () {
    ...
    //    var elem = $(e.target);
    //    if (elem.hasClass('blacksheet-main')) {
    //        $('.cancel-edit-profile').trigger("click");
    //    }
    //});
    ...
    $(document).delegate('.showcase_left_box', 'click', function () {
        if (sc_click_flag == 0) {
            ...
            $('.showcase_left_box').animate({ width: 450 }, function () {
                ...
                //$(this).closest('.showcase_step2').find('.step2_content_anchor').append('<textarea class="uploaddes_sc dotline_txtbox" placeholder="Title for the Showcase"></textarea>'
                //    + '<div>'
                ...
                //    + '</div>'
                //    + '<div class="driveafile"><div class="dv_txt">From Your Drive</div>'
                //    + '<a class="cf_bt"  id="chooseFromDrive">Choose File</a><div class="cf_prompt">No file chosen</div></div>'
                //    + '</div>'
                //    + '<div class="sc_footer"> <div class="sc_cancel">Cancel</div> <div class="sc_btn">Showcase</div><div class="sc_add" id="addShowcaseFile">Add</div></div>');
            });
            //            initDrivePicker();
        }
    });
    ...
    function DisplayProfileDetails(ary) {
        if ((ary['user_type'] != null) && (ary['user_type'].toLowerCase() == "p")) {
            ...
            $('#graduatingOn').css('margin-top', '12px');
            $('#graduatingOn').css('margin-left', '-68px');
        ...
        if ((ary['dept_name'] != null) && (ary['dept_name'] != "")) {
            $('#profile_dept_name').text(ary['dept_name']);

            $('#user_dept').val(ary['dept_name']);
        }
        else {
        ...
        $.ajax({
            ...
            success: function (responseText) {
                ...
                if (responseText.following.length > 0) {
                    followingList = DisplayConnections(responseText.following);
                    $('#followingCount').text(responseText.following.length);
                }

                if (responseText.followers.length > 0) {
                    followersList = DisplayConnections(responseText.followers);
                ...
                if (!is_owner) {
                    if (responseText.isfollowing) {
                        $('#user_connection').append('<a class = "follow unfollow" id="' + responseText.touser_id + '_' + responseText.fromuser_id + '">'
                        + 'Unfollow'
                        + '</a>');
                    }
                    else {
                        $('#user_connection').append('<a class = "follow" id="' + responseText.touser_id + '_' + responseText.fromuser_id + '">'
                        + 'Follow'
                        + '</a>');
                    }
    ...
    function DisplayConnections(ary) {
        var connectionsList = "";
...
+ '</div>';            
    ...
    $(document).delegate("#user_connection", "mouseover", function () {
        if ($(this).children('.follow').hasClass('unfollow')) {
            $(this).children('.follow').prop('text', 'Unfollow');
        }
    });

    $(document).delegate('#user_connection', 'click', function () {
        if ($(this).children('.follow').hasClass('unfollow')) {
            $(this).children('.follow').removeClass('unfollow');
            $(this).children('.follow').prop('text', 'Follow');
        }
        else {
            $(this).children('.follow').addClass('unfollow');
            $(this).children('.follow').prop('text', 'Following');
        }
        var targetID = $(this).prop('id').split('_')[0].trim();
    ...
    function FetchCourses(type) {
        ...
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "php/profile/professor_courses.php",
    ...
    function FetchClubs() {
        ...
        $.ajax({
            ...
            success: function (responseText) {
                ...
                if (responseText.user_clubs.length > 0) {
...
+ '</div>';
    ...
    function DisplayShowcase(ary) {
        ...
        var showcaseContent = '<li class="ddbox-invite-option ddbox-option-invited" id="invite_details_box4">';
        ...
        showcaseContent += '<div class="invite-option-title-wrap" style="display:none;"  title="' + ary['f_url'] + '"> <p>' + ary['file_desc'] + '</p></div></li>';
