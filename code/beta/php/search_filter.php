<?php
	...
	include_once('SPL.php');
   	...
   	/* $_POST['search_string'] = "daf";
   	$_POST['search_type']  = "Professor";
   	$_POST['page'] = 1;
   	$_POST['univid'] = 1; */
...
/*get everything with courses*/
	...
	if(isset($search_course_cond) && $search_course_cond!="")
	{
		...
		$course_sql = "SELECT gtable.course_id, gtable.dept_id, gtable.course_name,gtable.course_desc, gtable.course_credits,
					...
					AND gtable.course_id = cs.course_id)
					WHERE gtable.univ_id = ?
					AND upper(gtable.course_desc) LIKE ? ".$course_filter.$filter;
		$course_sql_stmt = $con->prepare($course_sql);
		...
		while($course_sql_stmt->fetch())
		{
			if(!(in_array_r($cclass_id, $course_array)))
			$course_array[] = array(
					'name'=>$cname,
					'dept_id' => $cdept_id,
					'id' => $cid,
	...
	if(count($class_ids) > 0)
	{
		...
		{ 
			if(isset($priority_array[$v['class']]))
				...
				$pq_course_array->insert($v, array(0,0));
		}
		$course_array = array();
	...
	if(count($user_id)>0)
	{
		...
		while($prof_sql_stmt->fetch())
		{
			$prof_array[] = array(
					...
					'picture' => $photo_prefix.$pic_loc."/".$pic,
					'desig' => $prof_desig,
					'location' => $prof_loc,
					'uname' => $uname
			);
	...
	if(isset($search_user_cond) && $search_user_cond!="")
	{
		...
		while($user_sql_stmt->fetch())
		{
			$user_array[] = array(
					...
					'bio'=> $ubio,
					'name'=>$ufname." ".$ulname,
					'picture'=>$photo_prefix.$upic_loc."/".$upic,
					'dept_id' => $udept_id,
					'uname'=>$uuniv_name
			);
...
/*----------------------------Filter results based on search type------------------------*/
	if($_POST['search_type'] != "All")
	{
		...
		if($_POST['search_type'] != "Professor" && isset($user_array))
		{
			$professor_rows = 0;
...
/*------------------------------VERTICAL-AREA----------------------------------------*/	
	...
	<h2>'.boldKeyWord($course_array[$i]['name']).'</h2>
	<p>'.boldKeyWord($course_array[$i]['id']).'</p>
	...
	<div class = "info-piece instructor">';
		...
		{
		echo boldKeyWord($course_array[$i]['prof']);
		}
		echo '</div>
		<div class = "info-piece subject">'.boldKeyWord($course_depts[$course_array[$i]['dept_id']]).'</div>
		...
		while ($memebers_sql_stmt->fetch())
		{
			...
			if($count <5)
			{
				...
				<div class = "smallPic">
				<img class = "img" src = "'.$photo_prefix.$mem_pic_loc."/".$mem_pic.'" width = "29" height = "29">
				</div>
				</a>';
			}
			$count += 1;
		...
		echo '<div class = "person vert-results-wrapper">
							...
							<div class = "person-main">
								<div class = "person-header">
									<div class = "result-header">
										<h2>'.boldKeyWord($row['name']).'</h2>
										<span>—</span>
										<p>'.boldKeyWord($course_depts[$row['dept_id']]).'</p>					
									</div>
									<div class = "result-header-right">
										<div class = "result-functions-wrapper">			
