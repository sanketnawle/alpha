...
//                });
    ...
    function FetchMembers(){
        //Load the members tab html template
        $members_tab_div = null;
        $.get(base_url + '/html/templates/members_tab.html',function(data){
            $members_tab_div = $(data);
        });
        ...
        $.getJSON(base_url + "/club/" + club_id.toString() + '/members', { group_id: club_id }, function(members_json) {
            ...
            $.each(members_json['members'],function(index, member){
                alert(JSON.stringify(member));
                //Clone the $member_div template
                //Notice how this easy this makes it to reuse code
                $temporary_member_div = $member_div.clone();
                $temporary_member_div.attr('id', member['user_id']);
                //Extract the data from json for ease of use
                var firstname = member['firstname'];
                var lastname = member['lastname'];
                var dp_link = member['dp_link'];

                //Access HTML attributes by using .find() on the $member_div
                $temporary_member_div.find('#member_name').text(firstname + ' ' + lastname);
                console.log("temp div");
                console.log($temporary_member_div);
                //$member_div.find('.img').attr('src',user['img_url'])

                //Add the member to the html
                $members_tab_div.find('#lol').append($temporary_member_div);
            });
            console.log($members_tab_div);



            //Add all the newly generated html to the members tab
            //After emptying it
            $(".members-tab-content").empty();
            $(".members-tab-content").append($members_tab_div);
        }).error(function() {
