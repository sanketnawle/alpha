        ...
        if($user){
            if($user->save(false)){
                ...
                if($user_recovery_test){
                    ...
                    ERunActions::touchUrl(Yii::app()->getBaseUrl(true) . '/site/sendVerificationEmailFunction',$postData=array('to_email'=>$user_email, 'subject'=>$subject, 'message'=>$message, 'from_email'=>$from, 'key'=>$user_recovery_test->key_email),$contentType=null);
                    $data = array('success'=>true);
                    $this->renderJSON($data);
                    return;
                }else{
