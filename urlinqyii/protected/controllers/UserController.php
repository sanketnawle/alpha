...
//        }
        ...
        $user_models = User::model()->findAllBySql('SELECT * FROM `user` WHERE user_id != ' . $user->user_id . ' and picture_file_id != 1 and university_id = ' . $university_id . ' LIMIT 10');
        ...
        //$users = $this->models_to_array();
        $data = array('success'=>true,'users'=>$user_models);
        $this->renderJSON($data);
        return;
    }
...
//        }
        ...
        //    $colors = array();
         ...
         /*       while(in_array($color->hex,$colors)){
            ...
            $user_data['departments'][$i] = $this->model_to_array($user->department);
            $user_data['departments'][$i]['color'] = array('hex'=>'#27e53f');
            $color = Color::model()->find('color_id=:id',array(':id'=>get_random_color()));
