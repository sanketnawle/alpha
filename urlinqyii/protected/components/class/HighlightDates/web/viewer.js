...
function highlightText(){
  ...
  var strings = [
                  "^((0[1-9])|(1[0-2]|[1-9]))(\/|-)([1-3]?[0-9])$", 
                  "^((0[1-9])|(1[0-2])|([1-9]))(\/|-)([0-3]?[0-9])(\/|-)((([0[1-9]])|([1-9][0-9]))|([1-9][0-9][0-9][0-9]))$",
                ]
  $.each(strings, function(index, value){
    rex[index]=new RegExp(value, 'ig');
  });

  $.each(text,function(index, value){
    $.each($(value).text().split(' '), function(ind, val){
      $.each(rex, function(re_index, re_value){
        if (val.match(re_value)){
          
          
          /*push date into seed repo*/
          var enc_val=$.md5($(value).text());
          var val_key=enc_val+"::"+val;

          var string_origin=$(".EVT_SEED_REPO").text();
          var so_arr=string_origin.split(";;");
          
          var inarr_flag= $.inArray(val_key,so_arr);

          /*highlight*/
          var after_rep=$(value).text().replace(val,'<span class="evt_seed_box" id='+enc_val+' style="background-color:red;">'+val+'</span>');
          $(value).html(after_rep);

          if (inarr_flag==-1) {

            $(".EVT_SEED_REPO").text($(".EVT_SEED_REPO").text()+val_key+";;"); 
          }
          /*push date into seed repo end*/
