...
if(isset($_POST['univid']) && $_POST['univid']!='')
{
	...
	$forumListQry = "select * from  university_posts_1 where univid='".$univid."' and (visibility='".$visibility."' OR visibility='Public') OR (`".$field_name."`='".$user_id."') order by update_timestamp desc ";
	...
	if(isset($forumListRes) && count($forumListRes)>0 && $forumListRes!=false)
	{
		...
		$forumreplyListQry = "select * from  university_reply_".$univid." where messageid='".$messageid."' order by update_timestamp desc ";
            ...
            <div class="p-main">
                ...
                <div class="doc">
                    <img src="images/doc-icon-2.png" id="docicon">
						...
						$sel_sql="SELECT count(*) as total_likes,univ_post_lkid  FROM `university_posts_likes_".$univid."` where postid='".$messageid."' and studentid='".$studentid."' and profid='".$profid."'";
				...
				if(isset($forumreplyListRes) && $forumreplyListRes!=false && count($forumreplyListRes)>0)
				{
					foreach($forumreplyListRes as $repcnt=>$replymsg)
					{
						...
						<div class="p-comment">
							...
							$sel_sql="SELECT count(*) as total_likes,univ_reply_lkid FROM `university_reply_likes_".$univid."` where replyid='".$replyid."' and studentid='".$studentid."' and profid='".$profid."'";
							$sel_cnt=$dbObj->fireQuery($sel_sql,'select');
							if($sel_cnt[0]['total_likes']>0)
							{
