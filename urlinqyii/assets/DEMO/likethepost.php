...
if(isset($_POST['messageid']) && $_POST['messageid']!='' && isset($_POST['type']) && $_POST['type']!='')
{
	...
	if($type=='post')
	{
		...
		{
			$update_sql="UPDATE `department_posts_1` SET like_cnt= like_cnt- 1 WHERE `messageid` = '".$messageid."'";
			$msg_id=$dbObj->fireQuery($update_sql,'update');
			$dele_ssql="DELETE FROM `department_posts_likes_1` WHERE `dep_post_lkid` = '".$sel_cnt[0]['dep_post_lkid']."'";
		...
		else
		{
			$update_sql="UPDATE `department_posts_1` SET like_cnt= like_cnt+ 1 WHERE `messageid` = '".$messageid."'";
			$msg_id=$dbObj->fireQuery($update_sql,'update');
			$insert_sql="INSERT INTO `department_posts_likes_1` (`studentid`,`profid`,`postid`) VALUES ('".$studentid."','".$profid."','".$messageid."');";
	...
	elseif($type=='reply')
	{
		...
		if($sel_cnt[0]['total_likes']>0)
		{
			$update_sql="UPDATE `department_reply_1` SET like_cnt= like_cnt- 1 WHERE `replyid` = '".$replyid."'";
			$msg_id=$dbObj->fireQuery($update_sql,'update');
			$dele_ssql="DELETE FROM `department_reply_likes_1` WHERE `dep_reply_lkid` = '".$sel_cnt[0]['dep_reply_lkid']."'";
			$msg_id=$dbObj->fireQuery($dele_ssql,'delete');
		}
		else
		{
			$update_sql="UPDATE `department_reply_1` SET like_cnt= like_cnt+ 1 WHERE `replyid` = '".$replyid."'";
			$msg_id=$dbObj->fireQuery($update_sql,'update');
			$insert_sql="INSERT INTO `department_reply_likes_1` (`studentid`,`profid`,`replyid`) VALUES ('".$studentid."','".$profid."','".$replyid."');";
			$msg_id=$dbObj->fireQuery($insert_sql,'insert');
		}
	} 
	if($msg_id>0)
