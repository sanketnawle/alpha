...
if (isset($_POST['create_schedule'])) {
    ...
    $insert_event_query = $con->prepare("INSERT INTO course_event (class_id, title, description, start_time, user_id, recurrence, end_time, start_date, end_date, location, event_class,made_by_admin ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
    if (!$insert_event_query) {
        echo "Error in preparing add user query: " . $con->error;
    }
    $insert_event_query->bind_param("ssssissssssi", $class_id, $title, $details, $start_time, $user_id, $repeat, $end_time, $start_date, $end_date, $location, $event_class, $made_by_admin);
    if (!$insert_event_query) {
        echo "Error in binding add user query: " . $con->error;
    }
...
//create event from the group page
if (isset($_POST['create_group_schedule'])) {
    ...
    $insert_event_query = $con->prepare("INSERT INTO group_event (group_id, title, description, start_time, user_id, recurrence, end_time, start_date, end_date, location, made_by_admin ) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
    if (!$insert_event_query) {
        echo "Error in preparing add user query: " . $con->error;
    }
    $insert_event_query->bind_param("isssisssssi", $group_id, $title, $details, $start_time, $user_id, $repeat, $end_time, $start_date, $end_date, $location, $made_by_admin);
    if (!$insert_event_query) {
        echo "Error in binding add user query: " . $con->error;
    }
    $insert_event_query->execute();
