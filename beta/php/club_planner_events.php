...
$tomorrow_date = $user_today_timestamp_copy->format("Y-m-d");
...
$class_events_query = "SELECT GE.* FROM group_event GE WHERE ((GE.event_id IN (SELECT GI.event_id FROM group_event_invited GI WHERE GI.user_id = $user_id AND GI.added = 1) AND GE.group_id = $group_id) OR (GE.group_id = $group_id AND GE.user_id = $user_id)) AND
((GE.start_date = '$today_date' AND GE.start_time >= '$now_time') OR (GE.start_date > '$today_date'))";
...
//        echo $event['type'];
        ...
        if ($prev_day !== $start_date) {
            ...
            } else if (isTomorrow1($start_date, $tomorrow_date)) {
