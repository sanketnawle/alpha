...
$group_event_query = "SELECT E.*, G.group_name, G.color_id FROM group_event E, groups G WHERE E.event_id IN
    (SELECT event_id FROM group_event_invited WHERE user_id='$user_id' AND (added='2' OR added='1')) AND (E.group_id NOT IN (SELECT GU.group_id FROM group_users GU WHERE GU.user_id = $user_id AND GU.is_admin = 1))
...
while ($row = mysqli_fetch_array($group_event_query_result)) {
    ...
    $get_color_query = "SELECT red_code ,green_code, blue_code FROM event_color_table WHERE color_id = $color_id";
...
$group_event_recurrence_query = "SELECT E.*, G.group_name, G.color_id FROM group_event E, groups G
        WHERE E.event_id IN (SELECT event_id FROM group_event_invited WHERE user_id='$user_id' 
            AND (added='2' OR added='1')) AND (E.group_id NOT IN (SELECT GU.group_id FROM group_users GU WHERE GU.user_id = $user_id AND GU.is_admin = 1))
            AND E.recurrence!='none' AND E.end_date >= '$month_start' AND E.group_id = G.group_id";
...
$course_event_query = "SELECT C.* FROM course_event C WHERE C.event_id IN
    (SELECT CI.event_id FROM course_event_invited CI WHERE CI.user_id='$user_id' AND (CI.choice='2' OR CI.choice='1'))
    AND (C.class_id NOT IN (SELECT CU.class_id FROM courses_user CU WHERE CU.user_id = $user_id AND CU.is_admin = 1))
        AND C.recurrence='none' AND C.start_date BETWEEN '$month_start' AND '$month_end'
        ORDER BY start_date ASC,start_time ASC";
...
$course_event_recurrence_query = "SELECT C.* FROM course_event C WHERE
    C.event_id IN (SELECT CI.event_id FROM course_event_invited CI WHERE CI.user_id='$user_id' AND (CI.choice='2' OR CI.choice='1')) AND (C.class_id NOT IN (SELECT CU.class_id FROM courses_user CU WHERE CU.user_id = $user_id AND CU.is_admin = 1))
        AND C.end_date >= '$month_start' AND C.recurrence!='none' ";
...
$course_personal_event_query = "SELECT * FROM course_event C WHERE ((C.user_id = $user_id) OR ((C.class_id IN (SELECT CU.class_id FROM courses_user CU WHERE CU.user_id = $user_id AND CU.is_admin = 1)) OR (C.class_id IN (SELECT CS.class_id FROM courses_semester CS WHERE CS.professor = $user_id)) AND C.made_by_admin = 1))
    AND C.start_date BETWEEN '$month_start' AND '$month_end'
        ORDER BY start_date ASC,start_time ASC";

$course_personal_event_query_result = mysqli_query($con, $course_personal_event_query);
//echo "course personal query: " . $course_personal_event_query;
...
$group_personal_event_query = "SELECT E.*, G.group_name, G.color_id FROM group_event E, groups G WHERE ((E.user_id = '$user_id') OR (E.group_id IN (SELECT GU.group_id FROM group_users GU WHERE GU.user_id = $user_id AND GU.is_admin = 1) AND E.made_by_admin = 1))
...
//echo "group personal event query: " . $group_personal_event_query;
