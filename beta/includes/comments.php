...
if(!function_exists('get_user_dp')){
	...
	if(77<$reply_len and $reply_len<157) $rlen_flag = "mid-len";
				...
				echo "<div class = 'post_comment ".$rlen_flag."' id = '".strtotime($row1['update_timestamp'])."'>";
					...
					if($cownership=="reply" or $cownership=="post"){
					...
					if($row1['user_id']!=0){
						$cowner_result=$con->query("select firstname, lastname from user where user_id='".$row1['user_id']."'");
						$cowner_row=$cowner_result->fetch_array();
						$comment_owner=$cowner_row['firstname']." ".$cowner_row['lastname'];
									
						// check if liked by the user and count the total votes
						$rquservotes=$con->query("SELECT * FROM reply_votes WHERE user_id='".$_SESSION['user_id']."' AND reply_id='".$row1['reply_id']."'"); //Checks current login user liked this status or not
						$ruservotes=$rquservotes->num_rows;
						$rtotalvotes=$con->query("SELECT * FROM reply_votes WHERE reply_id='".$row1['reply_id']."'");  // Total number of votes for the status message
						$replyvotes=$rtotalvotes->num_rows;				
					}

					else $comment_owner="Invalid User";

					echo "<div class = 'comment_main'>";
						if($row1['anon']!="1") echo "<a href = 'profile.php?user_id=".$row1['user_id']."'>";
								else "<a>";
							echo "<div class = 'comment_owner_container'>
								<div class = 'comment_user_icon' style='background:url(";
									if($row1['anon']=="0") echo get_user_dp($con,$row1['user_id']);
						...
						($cownership == 'reply' || $cownership == 'post'?"
							<div class='delete-comment' data-id='".$row1['reply_id']."'></div>":"").
