...
$get_course_details_query = "SELECT C.*, D.dept_name, D.dept_id, U.firstname, U.lastname, U.user_id, U.status, CM.location, CM.dp_blob_id, CM.cover_blob_id, COUNT(CU.user_id) as total  FROM courses_semester CM
...
while ($row = mysqli_fetch_array($get_course_details_query_result)) {
    ...
    echo '
    <div class = "group-head-sec" style="background-image:url(includes/get_blob.php?img_id=' . $row["cover_blob_id"] . ');background-size:cover;">
        <div class="group-head-content">
        <div class = "group-pic-frame">
             <div class = "group-pic" style="background-image:url(includes/get_blob.php?img_id=' . $row["cover_blob_id"] . 'background-size:cover;")>
            </div>
            <form>
