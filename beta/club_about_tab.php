...
$get_admin_flag_query = "SELECT is_admin as admin_flag FROM group_users WHERE user_id = ? AND is_admin = 1";
$get_admin_flag_stmt = $con->prepare($get_admin_flag_query);
if($get_admin_flag_stmt != null){
	$get_admin_flag_stmt->bind_param("i",$user_id);
...
if($get_course_about_stmt != NULL){	
    $get_course_about_stmt->bind_param("i", $group_id);	
    $get_course_about_stmt->execute();	
    $get_course_about_stmt->bind_result($group_desc);		
if($get_course_about_stmt->fetch())	{		
...
if (isset($admin_flag)) {
            ...
            </div>";	}$get_course_about_stmt->close();
}$connected_users = meFollowing($user_id);
...
if ($user_count > 0) {
    ...
    $get_club_student_query = "SELECT U.user_id, U.firstname, U.lastname FROM user U WHERE U.user_id IN(SELECT user_id from group_users
								  WHERE group_id = ? AND is_admin = 0 LIMIT 0,8)";
    $get_club_student_stmt = $con->prepare($get_club_student_query);    $club_admin = array();    if($get_club_student_stmt)    {		$get_club_student_stmt->bind_param('i', $group_id);		$get_club_student_stmt->execute();		$get_club_student_stmt->bind_result($g_user_id, $g_firstname, $g_lastname);		while($get_club_student_stmt->fetch())		{			$club_admin[] = array('user_id'=>$g_user_id, 'firstname'=>$g_firstname, 'lastname'=>$g_lastname);		}		$get_club_student_stmt->close();    }
    ...
    }}$get_club_admin_query = "SELECT U.user_id, U.firstname, U.lastname FROM user U WHERE U.user_id IN(SELECT user_id from group_users								  WHERE group_id = ? AND is_admin = 1 LIMIT 0,8)";$get_club_admin_stmt = $con->prepare($get_club_admin_query);if($get_club_admin_stmt){$get_club_admin_stmt->bind_param('i', $group_id);$get_club_admin_stmt->execute();$get_club_admin_stmt->bind_result($g_user_id, $g_firstname, $g_lastname);echo "<div class = 'about-tab-prof about-tab-block'>
...
echo "<div class = 'about-tab-rightsec'>
            <div class = 'group-about group-about-2'>";$get_course_files_query = "SELECT FU.file_id, FU.file_name, FU.created_timestamp, U.user_id, U.firstname, U.lastname FROM groups_files CF JOIN file_upload FU on FU.file_id = CF.file_id
...
LIMIT 0,1";$file_array = array();
$get_club_files_stmt = $con->prepare($get_course_files_query);if($get_club_files_stmt){	$get_club_files_stmt->bind_param("ii", $group_id, $group_id);	$get_club_files_stmt->execute();	$get_club_files_stmt->bind_result($g_file_id, $g_file_name,$g_created_time, $g_user_id, $g_firstname, $g_lastname);	while($get_club_files_stmt->fetch())	{		$file_array[] = array('file_id'=>$g_file_id, 'file_name'=>$g_file_name, 'created_timestamp'=>$g_created_time, 				'user_id'=>$g_user_id, 'firstname'=>$g_firstname, 'lastname'=>$g_lastname);	}	$get_club_files_stmt->close();}
