...
$user_id = 1;
$month_start_timestamp = new DateTime(server_time(date("Y-m-01 H:i:s", strtotime("now"))));
$month_start_date = $month_start_timestamp->format("Y-m-d");
$month_start_time = $month_start_timestamp->format("H:i:s");
$month_end_timestamp = new DateTime(server_time(date("Y-m-t H:i:s", strtotime("now"))));
...
$get_admin_event_query = "SELECT FROM group_event WHERE user_id IN (SELECT user_id FROM group_users WHERE group_id = $group_id AND is_admin = 1) AND ((start_date = $month_start_date AND start_time > $month_start_time) OR (start_date > $month_start_date)) AND (end_date = $month_end_date AND end_time < $month_end_time) OR (end_date < $month_end_date)))";
